...
adc_oneshot_unit_handle_t adc_handle[2]; // Array to hold ADC handles for each unit
...
{   
    ...
    for (int chan = 0; chan < 4; chan++) {
        res = adc_oneshot_read(adc_handle[0],ADC_CHANNEL_7, &val);
...
void initialize_adc()
{
    ...
    ESP_ERROR_CHECK(esp_timer_start_periodic(adc_timer, 1000)); // 1ms interval
